...
namespace mirada_finanza_control_central
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            // Ersetze SetDoubleBuffered durch diesen Aufruf im Konstruktor:
            // this.SetStyle(ControlStyles.OptimizedDoubleBuffer | ControlStyles.AllPaintingInWmPaint, true);

            // Im Konstruktor deiner Form nach InitializeComponent():
            /*
            SetDoubleBuffered(dataGridViewInvoices);
            SetDoubleBuffered(dataGridViewJournal);
            SetDoubleBuffered(dataGridViewAssets);
            SetDoubleBuffered(dataGridViewCustomers);
            SetDoubleBuffered(dataGridViewProducts);
            */

            // --- Grafik-Fix für ALLE Grids ---
            foreach (Control control in this.Controls)
            {
                // Wir suchen in allen Containern (Tabs, Panels) nach Grids
                ApplyGridGraphicsFix(control);
            }
        ...
        {
            this.SuspendLayout(); // Zeichnen einfrieren

            // Wir gehen durch alle TabPages deiner App
            foreach (TabPage page in tabControl.TabPages)
            {
                // Und in jeder Page suchen wir nach Grids
                foreach (Control ctrl in page.Controls)
                {
                    ...
                    // Falls das Grid in einem Panel oder einer GroupBox liegt
                    else if (ctrl.HasChildren)
                    {
                        foreach (Control child in ctrl.Controls)
                        {
                            if (child is DataGridView childDgv)
                            {
                                childDgv.Invalidate();
                                childDgv.Update();
                            }
            ...
            this.ResumeLayout(true); // Zeichnen wieder freigeben
        ...
        private void ApplyGridGraphicsFix(Control parent)
        {
            foreach (Control ctrl in parent.Controls)
            {
                if (ctrl is DataGridView dgv)
                {
                    ...
                    // 2. DIE WICHTIGEN ZEILEN FÜR DEN LINKEN RAND:
                    dgv.EnableHeadersVisualStyles = false; // Schaltet das Windows-Standard-Grau aus
                    dgv.RowHeadersVisible = true;          // Stellt sicher, dass der Rand da ist
                    dgv.RowHeadersDefaultCellStyle.SelectionBackColor = Color.DodgerBlue;
                    dgv.RowHeadersDefaultCellStyle.SelectionForeColor = Color.White;

                    dgv.RowPrePaint += (s, e) =>
                    {
                        // Killt den schwarzen Balken
                        e.PaintParts &= ~DataGridViewPaintParts.Focus;

                        // Killt das fehlerhafte Blau in den Zellen
